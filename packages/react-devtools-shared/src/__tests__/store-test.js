...
describe('Store', () => {
  ...
  it('should properly handle a root with no visible nodes', async () => {
    ...
    await act(() => legacyRender(<Root>{null}</Root>, container));
    expect(store).toMatchInlineSnapshot(`
      [root]
          <Root>
    `);

    await act(() => legacyRender(<div />, container));
  ...
  it('should handle multibyte character strings', async () => {
    ...
    await act(() => legacyRender(<Component />, container));
    expect(store).toMatchInlineSnapshot(`
      [root]
          <🟩💜🔵>
    `);
  ...
  describe('collapseNodesByDefault:false', () => {
    ...
    it('should support mount and update operations', async () => {
      ...
      await act(() => legacyRender(<Grandparent count={4} />, container));
      expect(store).toMatchInlineSnapshot(`
        [root]
          ▾ <Grandparent>
            ▾ <Parent>
      ...
      await act(() => legacyRender(<Grandparent count={2} />, container));
      expect(store).toMatchInlineSnapshot(`
        [root]
          ▾ <Grandparent>
            ▾ <Parent>
      ...
      await act(() => ReactDOM.unmountComponentAtNode(container));
      expect(store).toMatchInlineSnapshot(``);
    });
    ...
    it('should display Suspense nodes properly in various states', async () => {
      ...
      const container = document.createElement('div');
      await act(() =>
        legacyRender(<Wrapper shouldSuspense={true} />, container),
      );
      expect(store).toMatchInlineSnapshot(`
        [root]
          ▾ <Wrapper>
    ...
    it('should support reordering of children', async () => {
      ...
      await act(() => legacyRender(<Root>{[foo, bar]}</Root>, container));
      expect(store).toMatchInlineSnapshot(`
        [root]
          ▾ <Root>
            ▾ <Foo key="foo">
      ...
      await act(() => legacyRender(<Root>{[bar, foo]}</Root>, container));
      expect(store).toMatchInlineSnapshot(`
        [root]
          ▾ <Root>
            ▾ <Bar key="bar">
  ...
  describe('collapseNodesByDefault:true', () => {
    ...
    it('should not add new nodes when suspense is toggled', async () => {
      ...
      await act(() =>
        legacyRender(<SuspenseTree />, document.createElement('div')),
      );
      expect(store).toMatchInlineSnapshot(`
        [root]
          ▸ <SuspenseTree>
  ...
  // @reactVersion >= 18.0
  it('detects and updates profiling support based on the attached roots', async () => {
    const Component = () => null;
  ...
  it('should properly serialize non-string key values', async () => {
    ...
    await act(() => legacyRender([fauxElement], document.createElement('div')));
    expect(store).toMatchInlineSnapshot(`
      [root]
          <Child key="123">
    `);
  ...
  it('should show the right display names for special component types', async () => {
    ...
    await act(() => legacyRender(<App />, container));
  ...
  describe('inline errors and warnings', () => {
    ...
    it('during render are counted', () => {
      ...
      withErrorsOrWarningsIgnored(['test-only:'], async () => {
        await act(() => legacyRender(<Example />, container));
      });
      ...
      withErrorsOrWarningsIgnored(['test-only:'], async () => {
        await act(() => legacyRender(<Example rerender={1} />, container));
      });
    ...
    describe('during passive effects', () => {
      ...
      // @reactVersion >= 18.0
      it('are counted (after a delay)', async () => {
        function Example() {
          React.useEffect(() => {
        ...
        await act(flushPendingPassiveErrorAndWarningCounts, false);
        expect(store).toMatchInlineSnapshot(`
          ✕ 1, ⚠ 1
          [root]
              <Example> ✕⚠
    ...
    it('are updated when fibers are removed from the tree', () => {
      ...
      withErrorsOrWarningsIgnored(['test-only:'], async () => {
        await act(() => legacyRender(<React.Fragment />, container));
      });
      expect(store).toMatchInlineSnapshot(`[root]`);

...
1. [Publish release](./actions/workflows/runtime_release_from_ci.yml)
   ...
   - `type` is either `stable-latest` (e.g. for `react@latest`) or `stable-untagged` if this is for an older release that shouldn't be tagged.
